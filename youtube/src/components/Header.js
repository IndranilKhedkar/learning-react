...
export const Header = () => {
  ...
  useEffect(() => {
    const getSerachSuggestions = async () => {
      ...
      dispatch(cacheSearchResults({ [searchQuery]: result[1] }));
    };
    ...
    <div className="sticky top-0 bg-white">
      <div className="grid grid-flow-col p-4 shadow-lg items-center">
        ...
        <div className="col-span-10">
          <div>
            <input
              className="w-9/12 p-2 pl-5 border border-gray-400 rounded-l-full"
              type="text"
              placeholder="Search"
            ...
            />
            <button className="border border-gray-400 p-2 rounded-r-full bg-gray-200 px-6">
              🔍
            </button>
          </div>
          {showSuggestions && searchResults?.length > 0 && (
            <div className="w-[840px] p-5 my-1 rounded-3xl fixed bg-white shadow-md cursor-pointer">
              <ul>
                {searchResults.map((s) => (
                  <li key={s} className="py-1 m-1 hover:bg-slate-100">
                    {" "}
                    🔍 {s}
                  </li>
                ))}
